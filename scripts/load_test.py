#!/usr/bin/env python3
"""
HydraPay 10,000 TPS Load & Stress Test Simulator
------------------------------------------------
Fires up to 10,000+ concurrent transfer requests to the HydraPay /transfers endpoint
to validate 2-Tier Idempotency, Deterministic Account Row-Locking, and double-entry consistency.

Usage:
    python load_test.py [--url http://localhost:8080/api/v1/transfers] [--requests 10000] [--threads 50]
"""
...
# Seeded account IDs from V1__init_schema.sql
ACCOUNTS = [
...
def send_request(url, payload, idempotency_key):
    headers = {
...
def run_load_test(target_url, total_requests, thread_count, duplicate_ratio):
    print("==========================================================================")
    print("               HYDRAPAY 10,000 TPS LOAD & STRESS SIMULATOR                ")
    print("==========================================================================")
    ...
    # Pre-generate duplicate pool of keys to test idempotency hit rate
    ...
    for i in range(total_requests):
        # Pick from pool or create new key to simulate retries
        ...
        payload = {
            ...
            "amount": amount,
            "currency": "USD",
            "description": f"Load test tx #{i}"
        }
        work_items.append((payload, key))

    print(f"Starting load test execution across {thread_count} worker threads...")
    start_global = time.perf_counter()
    results = []

    with ThreadPoolExecutor(max_workers=thread_count) as executor:
        futures = [executor.submit(send_request, target_url, payload, key) for payload, key in work_items]
    ...
    print(f"Successful Requests (200): {len(successful)} ({(len(successful)/total_requests)*100:.1f}%)")
    print(f"  └─ Fresh Transactions  : {len(fresh)}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HydraPay Load Test Simulator")
    ...
    run_load_test(args.url, args.requests, args.threads, args.duplicates)
